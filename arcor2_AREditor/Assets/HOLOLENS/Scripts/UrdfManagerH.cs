...
public class UrdfManagerH : Singleton<UrdfManagerH> 
{
 /// <summary>
    ...
    private void ImportUrdfObject(string filename, string robotType) {
        ...
        RobotModelH robot = new RobotModelH(robotType, urdfRobot.gameObject);
        robot.LoadLinks();
    ...
    private void OnModelImported(object sender, ImportedModelEventArgs args) {
        ...
        if (urdfRobots != null) {
            ...
            RobotModelH robotModel = GetRobotModel(urdfRobot.gameObject);
            if (robotModel != null) {
    ...
    private RobotModelH CopyRobotModel(RobotModelH robotToCopy) {
    ...
    public RobotModelH GetRobotModelInstance(string robotType, string urdfDataPackageFilename = null) {
                ...
                if (RobotModels.TryGetValue(robotType, out List<RobotModelH> robotModels)) {
                    if (robotModels.Count > 0) {
                        ...
                        if (robotModels[0].RobotLoaded) {
                            ...
                            RobotModelH robotModel = CopyRobotModel(robotModels[0]);
    ...
    /// <param name="robotType">Type of the robot that will be imported.</param>
    /// <returns></returns>
    public RobotModelH GetRobotModel(string robotType) {
        if (RobotModels.TryGetValue(robotType, out List<RobotModelH> robotModels)) {
            if (robotModels.Count > 0) {
    ...
    /// <returns></returns>
    public RobotModelH GetRobotModel(GameObject robotModelGameObject) {
        foreach (List<RobotModelH> robotModels in RobotModels.Values) {
            foreach (RobotModelH robotModel in robotModels) {
    ...
    public void ReturnRobotModelInstace(RobotModelH robotModel) {
